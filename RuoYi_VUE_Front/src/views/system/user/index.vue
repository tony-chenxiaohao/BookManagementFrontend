    ...
    <el-row :gutter="25">
      ...
      <el-col :span="20" :xs="24">
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          />
<!--          :formatter="desensitizeName"-->
          <el-table-column label="部门" align="center" key="deptName" prop="dept.deptName" v-if="columns[3].visible"
                           :show-overflow-tooltip="true"/>
<!--          <el-table-column label="区域" align="center" key="regionName" prop="regionName" v-if="columns[6].visible"-->
<!--                           :show-overflow-tooltip="true"/>-->
          <el-table-column
            label="手机号码"
            align="center"
          ...
          />
<!--          :formatter="desensitizePhoneNumber"-->
...
<!--              <el-select v-model="form.regionId" placeholder="请选择归属区域">-->
<!--                <el-option-->
...
export default {
  ...
  methods: {
// 数据脱敏
    desensitizeName(row, column, cellValue) {
      if (!cellValue) return '';
      // 将姓名脱敏处理，例如 "张三" -> "张*"
      return cellValue.charAt(0) + '*';
    },
    desensitizePhoneNumber(row, column, cellValue) {
      if (!cellValue) return '';
      // 将手机号码脱敏处理，例如 "13812345678" -> "138****5678"
      return cellValue;
    ...
    handleResetPwd(row) {
      this.$prompt('请输入"' + row.nickName + '"的新密码', "提示", {
        ...
        getPublicKey().then(res => {
          // 假设 res.publicKey 是获取到的公钥
          // const publicKey = res.publicKey;
          // 对输入的密码进行加密
          // const encryptedPassword = encrypt(value, publicKey);

          // 调用接口重置用户密码，传递加密后的密码
          resetUserPwd(row.userId, value).then(response => {
            // this.$modal.msgSuccess("修改成功，新密码已加密传输。");
          });
        })
      }).catch(() => {});
    },
